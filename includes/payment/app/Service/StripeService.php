...
class StripeService
{
    ...
    public function retrieveStripeData($sessionId)
    {
        try {
            ...
            // When a customer cancels the checkout flow (or it otherwise remains incomplete),
            // Stripe does not create a PaymentIntent yet. In that case `payment_intent` is null
            // and attempting to retrieve it raises an exception. Instead, surface the session
            // data back to the caller so the application can treat it as a failed payment.
            if (empty($sessionData->payment_intent)) {
                $sessionStatus = $sessionData->payment_status ?? $sessionData->status ?? 'canceled';
                $sessionData->status = $sessionStatus;
                // Align the shape with the PaymentIntent object fields used later on.
                if (!isset($sessionData->amount)) {
                    $sessionData->amount = $sessionData->amount_total ?? 0;
                }
                return $sessionData;
            }
            ...
            // Attach the session details for reference (used when debugging).
            $paymentIntentData->checkout_session = $sessionData;
