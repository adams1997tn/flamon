...
// Check Razorpay payment process
    ...
    $customPayload = isset($requestData['custom2']) ? $requestData['custom2'] : '';
    $metadata = [];
    if ($customPayload !== '') {
    ...
    $orderId = $metadata['order_key'] ?? ($requestData['orderId'] ?? '');
    $amount = isset($metadata['amount']) ? $metadata['amount'] : ($requestData['initialPrice'] ?? ($requestData['amount'] ?? 0));
    $transactionId = $requestData['subscription_id'] ?? ($requestData['transId'] ?? null);
    $paymentResponseData = [
        'status'   => ($status === 'success' && !empty($metadata)),
        'rawData'  => $requestData,
        'data'     => preparePaymentData($orderId, $amount, $transactionId, 'ccbill')
    ];
    paymentResponse($paymentResponseData);
    ...
    $orderId = $monerooData['order_id'] ?? ($requestData['orderId'] ?? $requestData['order_id'] ?? null);
