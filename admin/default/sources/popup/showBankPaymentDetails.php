            ...
            <div class="i_delete_post_description column positionRelative">
                ...
                <div class="withdraw_other_details border_one flex_ column tabing">
                    <div class="withdrawl_detail_box flex_">
                        ...
                        <div class="withdrawl_detail_box_item_ f_bold">
                            <?php
                            $paymentAmount = $pData['amount'] ?? '';
                            $planPointAmount = null;
                            if (($paymentAmount === '' || $paymentAmount === null) && !empty($pData['credit_plan_id'])) {
                                $planData = $iN->GetPlanDetails((int) $pData['credit_plan_id']);
                                if (isset($planData['amount']) && $planData['amount'] !== '') {
                                    $paymentAmount = $planData['amount'];
                                }
                                $planPointAmount = $planData['plan_amount'] ?? null;
                            }
                            if (($paymentAmount === '' || $paymentAmount === null) && $planPointAmount !== null && $planPointAmount !== '') {
                                $pointValue = isset($onePointEqual) ? (string) $onePointEqual : '0';
                                $pointValue = (float) str_replace(',', '.', $pointValue);
                                $planPointValue = (float) str_replace(',', '.', (string) $planPointAmount);
                                if ($pointValue > 0 && $planPointValue > 0) {
                                    $paymentAmount = number_format($planPointValue * $pointValue, 2, '.', '');
                                }
                            }
                            echo $paymentAmount . $currencys[$defaultCurrency];
                            ?>
                        </div>
                    </div>
                ...
                <div class="withdraw_other_details border_one flex_ column tabing progme i_bank_detail_margin_top">
                    <div class="flex_ tabing">
                        <div class="appr c2 border_one transition tabing flex_ approve_bank_payment"
                             id="<?php echo iN_HelpSecure($pData['payer_iuid_fk']); ?>"
                             data-id="<?php echo iN_HelpSecure($planID); ?>"
                             data-imd="<?php echo iN_HelpSecure($ImageFileID); ?>"
                             data-pid="<?php echo iN_HelpSecure($paymentID); ?>"
