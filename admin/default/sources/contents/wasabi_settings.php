<?php
$wasRegionsList = isset($WASREGIONS) && is_array($WASREGIONS) ? $WASREGIONS : [];
$defaultWasRegionKey = '';
if (!empty($wasRegionsList)) {
    reset($wasRegionsList);
    $defaultWasRegionKey = key($wasRegionsList);
}
$activeWasRegionKey = (isset($WasRegion) && isset($wasRegionsList[$WasRegion])) ? $WasRegion : $defaultWasRegionKey;
$activeWasRegionLabel = $wasRegionsList[$activeWasRegionKey] ?? '';
?>
                    ...
                    <div class="irow_box_right">
                        <div class="i_box_limit flex_ column">
                            <div class="i_limit" data-type="s3update">
                                <span class="wasChoosed"><?php echo iN_HelpSecure($activeWasRegionLabel); ?></span>
                            ...
                            <div class="i_limit_list_s3_container">
                                <div class="i_countries_list border_one column flex_">
                                    <?php foreach ($WASREGIONS as $s3r => $value) { ?>
                                        <div class="i_s_limit transition border_one gsearch <?php echo iN_HelpSecure($activeWasRegionKey) == '' . $s3r . '' ? 'choosed' : ''; ?>" id="<?php echo iN_HelpSecure($s3r); ?>" data-c="<?php echo iN_HelpSecure($value); ?>" data-type="wasSet"><?php echo iN_HelpSecure($value); ?></div>
                                    <?php } ?>
                                </div>
                                <input type="hidden" name="wasregion" id="wasregion" value="<?php echo iN_HelpSecure($activeWasRegionKey); ?>">
                            </div>
                        </div>
                    </div>
                </div>
