            ...
            <div class="i_overflow_x_auto">
                <table class="border_one">
                    ...
                    foreach ($ApprovedPosts as $pay) {
                        ...
                        $paymentDataBankImage = $pay['bank_payment_image'] ?? null;
                        $paymentDataPaymentAmount = $pay['amount'] ?? null;
                        $paymentCreditPlanID = isset($pay['credit_plan_id']) ? (int) $pay['credit_plan_id'] : 0;

                        $planPointAmount = null;
                        if ($paymentCreditPlanID > 0) {
                            $planData = $iN->GetPlanDetails($paymentCreditPlanID);
                            if (isset($planData['amount']) && $planData['amount'] !== '') {
                                $paymentDataPaymentAmount = $planData['amount'];
                            }
                            $planPointAmount = $planData['plan_amount'] ?? null;
                        }
                        if (($paymentDataPaymentAmount === null || $paymentDataPaymentAmount === '')
                            && $paymentDataPaymentType === 'point'
                            && $paymentDataPaymentOption === 'bank'
                            && $planPointAmount !== null
                            && $planPointAmount !== ''
                        ) {
                            ...
                            if ($pointValue > 0 && $planPointValue > 0) {
                                $paymentDataPaymentAmount = number_format($planPointValue * $pointValue, 2, '.', '');
                            }
                        }
                        ...
                        if ($paymentDataPaymentStatus == 'pending') {
                            if ($paymentDataPaymentOption === 'bank' && !empty($paymentDataBankImage)) {
