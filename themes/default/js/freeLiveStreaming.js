(function ($) {
  ...
  ensureProvider(function startLive() {
    ...
    async function join() {
      ...
      if (options.role === "host") {
        localTracks.audioTrack = await provider.createMicrophoneTrack({ microphoneId: currentMic?.deviceId });
        localTracks.videoTrack = await provider.createCameraTrack({ cameraId: currentCam?.deviceId });
        ...
        // Try loading device list after tracks are created (permissions are usually granted by then).
        try {
          await mediaDeviceTest();
        } catch (err) {
          console.warn(err);
  ...
  $(function () {
    ...
    if (window.liveUserID == window.liveCreator) {
      $("body").on("click", ".camcloseCall", function () {
        ...
        $(".i_modal_bg_in").remove(); // prevent stacked modals
        $.ajax({
          type: "POST",
          url: `${window.siteurl}requests/request.php`,
          data: data,
