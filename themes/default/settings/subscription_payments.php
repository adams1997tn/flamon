<?php
$totalSubscriptionPayments = (int) DB::col(
    "SELECT COUNT(*) FROM i_user_subscriptions
     WHERE subscribed_iuid_fk = ? AND status IN('active','inactive') AND in_status IN('1','0') AND finished IN('0','1')
       AND subscription_scope IN ('profile','community')",
    [(int)$userID]
);
$totalPages = ceil($totalSubscriptionPayments / $paginationLimit);
    ...
    <div class="i_settings_wrapper_items">
       ...
       <div class="i_tab_container">
           ...
           <div class="i_tab_header flex_">
               ...
               <div class="tab_item item_mobile"><?php echo iN_HelpSecure($LANG['subscription_scope']);?></div>
           ...
           <div class="i_tab_list_item_container">
            <?php
               ...
               if($payedActiveSubscriptions){
                  foreach($payedActiveSubscriptions as $payedSub){
                    ...
                    $agencyEarning = $payedSub['agency_earning'] ?? null;
                    $subscriptionStatus = $payedSub['status'];
                    $subscriptionScope = $payedSub['subscription_scope'] ?? 'profile';
                    $scopeLabel = $LANG['subscription_scope_' . $subscriptionScope] ?? $LANG['subscription_scope_profile'];
            ...
            ?>
                ...
                <div class="i_tab_list_item flex_">
                    ...
                    <div class="tab_detail_item item_mobile"><?php echo iN_HelpSecure($scopeLabel);?></div>
                    ...
                    <div class="tab_detail_item"><?php echo iN_HelpSecure(formatCurrency((float)$userNetEarningC, $payedCurrency));?></div>
     ...
     <div class="i_become_creator_box_footer tabing">
        <?php if (ceil($totalSubscriptionPayments / $paginationLimit) > 0): ?>
            <ul class="pagination">
                <?php if ($pagep > 1): ?>
                ...
                <?php if ($pagep+1 < ceil($totalSubscriptionPayments / $paginationLimit)+1): ?><li class="page"><a class="transition" href="<?php echo iN_HelpSecure($base_url);?>settings?tab=subscription_payments&page-id=<?php echo iN_HelpSecure($pagep)+1; ?>"><?php echo iN_HelpSecure($pagep)+1; ?></a></li><?php endif; ?>
                <?php if ($pagep+2 < ceil($totalSubscriptionPayments / $paginationLimit)+1): ?><li class="page"><a class="transition" href="<?php echo iN_HelpSecure($base_url);?>settings?tab=subscription_payments&page-id=<?php echo iN_HelpSecure($pagep)+2; ?>"><?php echo iN_HelpSecure($pagep)+2; ?></a></li><?php endif; ?>

                <?php if ($pagep < ceil($totalSubscriptionPayments / $paginationLimit)-2): ?>
                <li class="dots">...</li>
                <li class="end"><a class="transition" href="<?php echo iN_HelpSecure($base_url);?>settings?tab=subscription_payments&page-id=<?php echo ceil($totalSubscriptionPayments / $paginationLimit); ?>"><?php echo ceil($totalSubscriptionPayments / $paginationLimit); ?></a></li>
                <?php endif; ?>

                <?php if ($pagep < ceil($totalSubscriptionPayments / $paginationLimit)): ?>
