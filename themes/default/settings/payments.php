    ...
    <div class="i_settings_wrapper_items">
       <div class="i_tab_container">
           ...
           <div class="i_tab_header flex_">
               ...
               <div class="tab_item item_mobile"><?php echo iN_HelpSecure($LANG['invoice_number']);?></div>
           </div>
           <!--/PAYMENTS TABLE HEADER-->
           <!---->
            ...
            <?php
               ...
               if($paymentHistory){
                  foreach($paymentHistory as $pay){
                    ...
                    $paymentDataPaymentAgencyEarning = $pay['agency_earning'] ?? null;
                    $paymentDataPaymentUserEarning = $pay['user_earning'] ?? null;
                    $currencyCode = $pay['currency'] ?? $defaultCurrency;
                    $currencySymbolItem = $currencys[$currencyCode] ?? $currencys[$defaultCurrency];
                    $invoiceToken = $pay['invoice_token'] ?? null;
                    $invoiceNumber = $pay['invoice_number'] ?? null;
                    ...
                    $isWalletTopUp = $paymentDataPaymentType === 'point';
                    $paymentTypeLabel = $PAYMENTTYPES[$paymentDataPaymentType] ?? ($LANG[$paymentDataPaymentType] ?? ucwords(str_replace('_', ' ', (string)$paymentDataPaymentType)));
                    if ($isWalletTopUp) {
                        $paymentTypeLabel = $LANG['wallet_top_up'];
                    }
                    $invoiceLinkLabel = (string)$invoiceNumber;
                    if ($invoiceLinkLabel === '' || strpos($invoiceLinkLabel, '_') !== false || strlen($invoiceLinkLabel) > 24) {
                        $invoiceLinkLabel = $LANG['invoice_number'] ?? ($LANG['invoice'] ?? 'Invoice');
                    }
            ?>
                <!--ITEM-->
