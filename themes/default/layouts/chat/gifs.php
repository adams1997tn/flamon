<div class="Message_stickersContainer">
    <div class="Message_stickers_wrapper">
        ...
        <div class="giphy_search_bar" style="display:flex;gap:8px;align-items:center;margin-bottom:10px;">
            <div class="giphy_search_form" data-request-type="chat_gifs" data-id="<?php echo iN_HelpSecure($id); ?>" style="display:flex;gap:8px;width:100%;" role="search" aria-label="Giphy search">
                <input
                    type="search"
                    name="giphy_query"
        ...
        <div class="giphy_results_container_conversation">
            <?php
            if (!$giphyEnabled) {
                echo '<div class="no-gif-found">' . iN_HelpSecure($LANG['giphy_disabled']) . '</div>';
            } elseif (!empty($giphyKey)) {
                if ($isSearchMode) {
                    $apiUrl = 'https://api.giphy.com/v1/gifs/search?api_key=' . urlencode($giphyKey) . '&limit=25&rating=pg&q=' . urlencode($searchQuery);
                } else {
                ...
                if (curl_errno($ch) || !$response) {
                    echo '<div class="no-gif-found">' . iN_HelpSecure($LANG['could_not_retrieve_gifs'] ?? $LANG['could_not_fined_gif']) . '</div>';
                } else {
                    $json = json_decode($response);

                    if (!isset($json->data) || !is_array($json->data) || count($json->data) === 0) {
                        echo '<div class="no-gif-found">' . iN_HelpSecure($searchEmptyText) . '</div>';
                    } else {
                ...
                echo '<div class="no-gif-found">' . iN_HelpSecure($LANG['please_add_giphy_api_key_from_admin']) . '</div>';
            }
            ?>
        </div>
    </div>
