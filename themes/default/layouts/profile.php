...
?>
	...
	<div class="profile_wrapper" id="prw" data-u="<?php echo iN_HelpSecure($p_profileID);?>" data-connections-visibility="<?php echo isset($p_connectionsVisibility) ? iN_HelpSecure($p_connectionsVisibility) : '1'; ?>" data-owner="<?php echo ($p_profileID == $userID) ? '1' : '0'; ?>">
	    ...
	    if (isset($_GET['pcat']) && $_GET['pcat'] !== '') {
	        $pcatParam = trim((string) $_GET['pcat']);
	        if (in_array($pcatParam, $pCats, true)) {
	            $pCat = $pcatParam;
	        }
	    }
...
<?php if ($logedIn != 0) { ?>
<script type="text/javascript" src="<?php echo iN_HelpSecure($base_url); ?>themes/<?php echo iN_HelpSecure($currentTheme); ?>/js/sw/lib/hammer.min.js?v=<?php echo iN_HelpSecure($version); ?>"></script>
<script type="text/javascript" src="<?php echo iN_HelpSecure($base_url); ?>themes/<?php echo iN_HelpSecure($currentTheme); ?>/js/sw/story-view.js?v=<?php echo iN_HelpSecure($version); ?>"></script>
<script type="text/javascript" src="<?php echo iN_HelpSecure($base_url); ?>themes/<?php echo iN_HelpSecure($currentTheme); ?>/js/sw/storyHandler.js?v=<?php echo iN_HelpSecure($version); ?>"></script>
<script type="text/javascript" src="<?php echo iN_HelpSecure($base_url); ?>themes/<?php echo iN_HelpSecure($currentTheme); ?>/js/highlightHandler.js?v=<?php echo iN_HelpSecure($version); ?>"></script>
<?php } ?>
</body>
</html>
