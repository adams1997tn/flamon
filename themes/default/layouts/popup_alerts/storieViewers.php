<div class="i_modal_bg_in" role="dialog" aria-modal="true" aria-labelledby="storieViewersModalTitle">
    ...
    <div class="i_modal_in_in i_sf_box">
        <div class="i_modal_content">
            ...
            <?php
                $storySeenCount = isset($storySeenCount) ? (int)$storySeenCount : 0;
                $storyReplyCount = isset($storyReplyCount) ? (int)$storyReplyCount : 0;
                $storyReactionTotal = isset($storyReactionTotal) ? (int)$storyReactionTotal : 0;
            ?>
            <div class="story_analytics">
                <div class="story_analytics_item"><?php echo iN_HelpSecure($LANG['story_views_label']); ?> <span><?php echo $storySeenCount; ?></span></div>
                <div class="story_analytics_item"><?php echo iN_HelpSecure($LANG['story_replies_label']); ?> <span><?php echo $storyReplyCount; ?></span></div>
                <div class="story_analytics_item"><?php echo iN_HelpSecure($LANG['story_reactions_label']); ?> <span><?php echo $storyReactionTotal; ?></span></div>
            </div>
            <?php if (!empty($storyReactionSummary)) { ?>
            <div class="story_reaction_summary">
                <?php foreach ($storyReactionSummary as $reactionRow) {
                    $reaction = $reactionRow['reaction'] ?? '';
                    $total = (int)($reactionRow['total'] ?? 0);
                    if ($reaction === '' || $total <= 0) { continue; }
                ?>
                    <span class="story_reaction_badge"><?php echo iN_HelpSecure($reaction); ?> <?php echo $total; ?></span>
                <?php } ?>
            </div>
            <?php } ?>
