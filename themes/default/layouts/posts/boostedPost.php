...
// Force fallback query (no friend filter) to ensure we render an active boost
$BoostedPost = $iN->iN_ShowBoostedPostFallback($logedIn === '1' ? $userID : 0);
// Normalize single-row result to array of rows
if ($BoostedPost && isset($BoostedPost['post_id'])) {
    $BoostedPost = [$BoostedPost];
}
$boostDebug = [
    'logedIn' => $logedIn,
    'userID' => isset($userID) ? $userID : null,
    'boosted_status' => isset($boostedPostEnableDisable) ? $boostedPostEnableDisable : null,
    'site_boost_count' => $iN->iN_CheckHaveBoostedPostAllTheSite(),
    'fallback_result_count' => is_array($BoostedPost) ? count($BoostedPost) : 0,
];
...
if($BoostedPost){
   foreach($BoostedPost as $postFromData){
    ...
    $planIcon = '';
    $checkCount = 0;
    $viewCount = isset($postFromData['view_count']) ? (int)$postFromData['view_count'] : 0;
    $boostPlanID = $postFromData['boost_type'] ?? null;
    $boostStatus = $postFromData['status'] ?? null;
    $boostID = $postFromData['boost_id'] ?? null;
    $boostPostOwnerID = $postFromData['iuid_fk'] ?? null;
    if ($boostPlanID) {
    ...
    if($logedIn != 0 && $boostID && $boostStatus === 'yes' && $userID != $boostPostOwnerID){
        $userIP = $iN->iN_GetIPAddress();
        $checkCount = (int)$iN->iN_CountSeenBoostedPostbyID($boostPostOwnerID,$boostID);
        if($viewCount === 0 || $checkCount < $viewCount){
            $iN->iN_BoostPostSeenCounter($userID, $boostID, $userIP);
        } else {
            $iN->iN_UpdateBoostedPostStatus($boostPostOwnerID, $boostID);
        }
    }
