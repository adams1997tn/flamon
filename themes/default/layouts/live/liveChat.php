...
if (!empty($liveMessageList)) {
    foreach ($liveMessageList as $lmData) {
        ...
        $getLiveGiftDataFromID = null;
        $giftImage = '';
        $giftAnimationUrl = '';
        if (!empty($giftSended)) {
            $getLiveGiftDataFromID = $iN->GetLivePlanDetails($giftSended);
            $giftImage = $getLiveGiftDataFromID['gift_image'] ?? '';
            $giftAnimationImage = $getLiveGiftDataFromID['gift_money_animation_image'] ?? '';
            if (!empty($giftAnimationImage)) {
        ...
        $giftAnimationAttr = $giftAnimationUrl ? ' data-gift-anim="' . iN_HelpSecure($giftAnimationUrl) . '" data-gift-played="1"' : '';
?>
        <div class="gElp9 flex_ tabing_non_justify eo2As cUq_<?php echo $safeMessageID; ?>" id="<?php echo $safeMessageID; ?>"<?php echo $giftAnimationAttr; ?>>
