...
$metaUrl = $base_url . $p_username;
$metaImage = $p_profileAvatar;
...
$p_connectionsVisibility = isset($profileData['connections_visibility']) ? (string)$profileData['connections_visibility'] : '1';
...
if($blockedType == '2'){
   include("sources/404.php");
    ...
    if (isset($agencyModuleStatus) && $agencyModuleStatus === 'yes') {
        $boostClickId = isset($_GET['ab']) ? (int)$iN->iN_Secure($_GET['ab']) : 0;
        if ($boostClickId > 0) {
            $boost = $iN->iN_GetAgencyBoostById($boostClickId);
            if ($boost && (int)($boost['creator_iuid_fk'] ?? 0) === (int)$p_profileID) {
                $boostAgencyId = (int)($boost['agency_id_fk'] ?? 0);
                if ($boostAgencyId > 0) {
                    $viewerId = ($logedIn == 1) ? (int)$userID : null;
                    $viewerIp = (string)$iN->iN_GetIPAddress();
                    $iN->iN_LogAgencyBoostClick($boostClickId, $boostAgencyId, (int)$p_profileID, $viewerId, $viewerIp);
                }
            }
        }
    }
    if ($logedIn == 1 && $userID != $p_profileID && $checkUserisCreator == 1) {
        $iN->iN_SendCreatorAutoMessageIfEligible($p_profileID, $userID);
    }
    $page = 'profile';
